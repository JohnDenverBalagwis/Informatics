...
if (isset($_POST['submit'])) {
    while ($row = mysqli_fetch_assoc($candidates)) {
        ...
        $judges->insertData('male_formal_attire', ['male_candidate_id'=>$id, 'poise_and_bearing'=>$poise_and_bearing, 'stage_presence'=>$stage_presence, 'fitness_and_style'=>$fitness_and_style, 'elegance'=>$elegance]);
    }

    header("location: before-qa.php");
}
...
<body>
    <nav class="top-nav">
        ...
        <h3 class="title-category">Formal Attire Male 25%</h3>
    </nav>
            ...
            <table class="table table-striped candidate-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <input style="width: 4.6rem;" class="candidate-input form-control mx-auto oneHundred" type="number" name="stage-presence<?php echo $row['id']; ?>" required>
                        </td>
                        <td>
                            <input style="width: 4.6rem;" class="candidate-input form-control mx-auto oneHundred" type="number" name="fitness-and-style<?php echo $row['id']; ?>" required>
                        </td>
                        <td>
                            <input style="width: 4.6rem;" class="candidate-input form-control mx-auto oneHundred" type="number" name="elegance<?php echo $row['id']; ?>" required>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
    ...
    <?php 
        if (!isset($_SESSION['formal-attire-male'])) {
            echo "<script>showPopup('Submitted Sucessfully');</script>";
            $_SESSION['formal-attire-male'] = true;
        }
    ?>
</body>
